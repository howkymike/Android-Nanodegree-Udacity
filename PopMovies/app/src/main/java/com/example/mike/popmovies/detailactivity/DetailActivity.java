...
public class DetailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Should it be in AsyncTask? (database is relatively small)
        ...
        switch (item.getItemId()) {
            case R.id.menuitem_favourite:
                ...
                if ( itemIcon.getConstantState().equals(getDrawable(R.drawable.ic_not_favourite).getConstantState()) ) {
                    ...
                    long newRowId = addMovieToDb(movie.getID(), movie.getTitle(), movie.getOverview(), movie.getVote_average(), movie.getRelease_date());
                    ...
                    boolean ifDeleted = deleteMovieFromDb(movie.getID());
                    Log.v(TAG, "Deleting status:" + ifDeleted);
    ...
    private void checkFavouriteFromDB(Menu menu) {
        int favourite = 0;
        ...
        Cursor allDataCursor = mDb.query(
    ...
    private long addMovieToDb(int movie_id, String title, String overview, double rating, String release_date) {
        ...
        return mDb.insert(MovieDbContract.MovieDbEntry.TABLE_NAME, null, cv);
    }

    private boolean deleteMovieFromDb(long id) {
        return mDb.delete(MovieDbContract.MovieDbEntry.TABLE_NAME, MovieDbContract.MovieDbEntry.COLUMN_MOVIE_ID + "=" + id, null) > 0;
    }
}
